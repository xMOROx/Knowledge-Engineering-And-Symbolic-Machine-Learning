...
generate_battle_file() {
  ...
  if [ -z "$ROBOCODE_MY_ROBOT_NAME" ]; then
    log_error "ROBOCODE_MY_ROBOT_NAME is not set. Cannot generate battle file."
    return 1
  fi
...
EOF
  ...
  if [ -z "$opponent_list" ]; then
    log_warn "No opponents specified in config! Running only against self?"
    opponent_list=""
  fi
...
start_server() {
  # Use global SERVER_LOG, SERVER_DIR, SERVER_PYTHON_EXE, SERVER_SCRIPT_NAME,
  # SERVER_IP, SERVER_LEARN_PORT, SERVER_WEIGHT_PORT, LOG_DIR, LOGGING_PYTHON_LOG_LEVEL
  # Modifies global SERVER_PID
...
wait_for_server() {
  # Use global SERVER_IP, SERVER_LEARN_PORT, SERVER_WEIGHT_PORT, SERVER_LOG
  local ip=$SERVER_IP
...
start_robocode_instance() {
  local instance_id=$1
  # Use global LOG_DIR, ROBOT_BIN_DIR, ROBOT_LIBS_DIR, ROBOCODE_HOME, PROJECT_LIBS_DIR,
  # GENERATED_BATTLE_FILE_PATH, ROBOCODE_TPS, ROBOCODE_GUI
  # Modifies global ROBOCODE_PIDS array
...
tail_log() {
  local log_file="$1"
  local prefix="$2"
  # Use global FLAG_TAIL_LOGS
  # Modifies global LOG_TAIL_PIDS array
  if ! ${FLAG_TAIL_LOGS}; then return 0; fi

  tail -F --pid=$$ --retry "$log_file" 2>/dev/null | while IFS= read -r line; do
    if [[ -n "${line// /}" ]]; then echo -e "${prefix} ${line}"; fi
  done &
  LOG_TAIL_PIDS+=("$!")
}
